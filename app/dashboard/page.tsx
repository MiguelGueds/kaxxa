...
export default function DashboardPage() {
  ...
  const accountBalance = (account: DbAccount) => {
    ...
    const linkedMovement = transactions
      .filter(transaction => transaction.account_id === account.id && transaction.is_paid !== false)
      .reduce((sum, transaction) => sum + (transaction.type === 'INCOME' ? 1 : -1) * Math.abs(Number(transaction.amount || 0)), 0);

    return initialBalance + linkedMovement;
  };
  const reconciledAccountsTotal = accounts.reduce((sum, account) => sum + accountBalance(account), 0);
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] space-y-5 pb-12">
      ...
      <div className="grid gap-5 xl:grid-cols-[0.95fr_1.45fr]">
        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><div><h2 className="text-sm font-semibold text-slate-800">My Wallet</h2><p className="text-[11px] text-slate-400">Contas bancárias conectadas</p></div><Link href="/dashboard/configuracoes" className="inline-flex items-center gap-1 text-[11px] text-slate-500"><Plus className="h-3.5 w-3.5" /> Add New</Link></div><div className="grid grid-cols-2 gap-3">{accounts.length === 0 ? <div className="col-span-2 rounded-lg bg-slate-50 p-6 text-center text-xs text-slate-400">Nenhuma conta cadastrada.</div> : accounts.map(account => { const balance = accountBalance(account); return <Link href="/dashboard/transacoes" key={account.id} className="rounded-lg border border-slate-100 bg-slate-50 p-3 transition hover:border-emerald-200"><div className="flex items-center justify-between"><BankLogo name={account.name} size="sm" /><span className="text-[10px] text-slate-400">{account.type}</span></div><p className="mt-3 truncate text-xs font-semibold text-slate-700">{account.name}</p><p className={`mt-1 text-sm font-semibold ${balance < 0 ? 'text-rose-600' : 'text-slate-900'}`}>R$ {money(balance, isConcealed)}</p><span className="text-[10px] text-emerald-600">Ativa</span></Link>; })}</div></section>

        <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-sm font-semibold text-slate-800">Cash Flow</h2><p className="text-[11px] text-slate-400">Entradas e saídas consolidadas</p></div><div className="flex items-center gap-2"><span className="text-xl font-semibold text-slate-900">R$ {money(reconciledAccountsTotal + totalInvestments - totalDebts, isConcealed)}</span><BarChart3 className="h-4 w-4 text-emerald-600" /></div></div><div className="flex h-48 items-end gap-3 border-b border-slate-100 px-2">{flow.map((item, index) => <div key={item.month} className="flex flex-1 flex-col items-center gap-2"><div className={`w-full max-w-10 rounded-t-md transition-all ${index === 2 ? 'bg-emerald-600' : 'bg-emerald-100'}`} style={{ height: `${Math.max(8, (item.value / maxFlow) * 135)}px` }} title={`R$ ${money(item.value, isConcealed)}`} /><span className="text-[10px] text-slate-400">{item.month}</span></div>)}</div><div className="mt-4 flex items-center justify-between text-[11px] text-slate-400"><span>Saldo em contas: R$ {money(reconciledAccountsTotal, isConcealed)}</span><span>Cartões: R$ {money(totalCardUsed, isConcealed)} / {money(totalCardLimit, isConcealed)}</span></div></section>
      </div>
